// Sidebar Panel Script - Handles UI interactions, Excel upload, Page Markers, and Assertions
...
const azureSasToken = document.getElementById('azureSasToken');
const azureStatus = document.getElementById('azureStatus');
const testAzureBtn = document.getElementById('testAzureBtn');
const saveAzureBtn = document.getElementById('saveAzureBtn');
const refreshCloudFilesBtn = document.getElementById('refreshCloudFilesBtn');
...
function setupEventListeners() {
  ...
  saveAzureBtn?.addEventListener('click', saveAzureConfiguration);
  refreshCloudFilesBtn?.addEventListener('click', refreshCloudFiles);
...
function setupExcelUpload() {
  ...
  document.getElementById('uploadAllToCloudBtn')?.addEventListener('click', uploadAllFilesToCloud);
  
  // Update Azure sync status
  updateAzureSyncStatus();
}
...
function openSettingsModal() {
  ...
  if (window.AzureStorage) {
    azureAccountName.value = window.AzureStorage.AZURE_CONFIG.accountName !== 'YOUR_STORAGE_ACCOUNT_NAME' 
      ? window.AzureStorage.AZURE_CONFIG.accountName : '';
    azureSasToken.value = window.AzureStorage.AZURE_CONFIG.sasToken !== 'YOUR_SAS_TOKEN_HERE'
      ? window.AzureStorage.AZURE_CONFIG.sasToken : '';
  }
  updateAzureStatusUI();
}
...
async function loadAzureConfiguration() {
  if (window.AzureStorage) {
    await window.AzureStorage.loadAzureConfig();
    updateAzureStatusUI();
    
    // If configured, refresh cloud files
    if (window.AzureStorage.isAzureConfigured()) {
      refreshCloudFiles();
    }
  }
}

function updateAzureStatusUI() {
  if (!window.AzureStorage || !window.AzureStorage.isAzureConfigured()) {
    ...
    azureStatus.innerHTML = `
      ...
      <span class="status-text">Connected to ${window.AzureStorage.AZURE_CONFIG.accountName}</span>
    `;
  }
}

async function testAzureConnection() {
  const accountName = azureAccountName.value.trim();
  const sasToken = azureSasToken.value.trim();
  
  if (!accountName || !sasToken) {
    alert('Please enter both Storage Account Name and SAS Token');
    return;
  }
  ...
  azureStatus.innerHTML = `
    ...
    <span class="status-text">Testing connection...</span>
  `;
  
  // Temporarily configure
  window.AzureStorage.configureAzureStorage(accountName, sasToken);
  
  try {
    // Try to list files in the excel-data container
    const files = await window.AzureStorage.listAzureBlobs('excel-data');
    
    azureStatus.innerHTML = `
      <span class="status-indicator connected"></span>
      <span class="status-text">Connection successful! Found ${files.length} files</span>
    `;
  } catch (error) {
    azureStatus.innerHTML = `
...
async function saveAzureConfiguration() {
  const accountName = azureAccountName.value.trim();
  const sasToken = azureSasToken.value.trim();
...
async function refreshCloudFiles() {
  if (!window.AzureStorage || !window.AzureStorage.isAzureConfigured()) {
    cloudFilesList.innerHTML = '<p class="no-files">Configure Azure Storage to view cloud files</p>';
    return;
  }
  
  cloudFilesList.innerHTML = '<p class="no-files">Loading...</p>';
  
  try {
    const files = await window.AzureStorage.listAzureBlobs('excel-data');
    
    if (files.length === 0) {
      cloudFilesList.innerHTML = '<p class="no-files">No files in cloud storage</p>';
      return;
    }
    
    cloudFilesList.innerHTML = files.map(file => `
      <div class="cloud-file-item" data-name="${escapeHtml(file.name)}">
        <div>
          <div class="cloud-file-name">${escapeHtml(file.name)}</div>
          <div class="cloud-file-size">${formatFileSize(file.size)}</div>
        </div>
    ...
    // Add event listeners
    cloudFilesList.querySelectorAll('.cloud-file-item').forEach(item => {
      const fileName = item.dataset.name;
      
      item.querySelector('.download-btn')?.addEventListener('click', async (e) => {
        e.stopPropagation();
        await downloadCloudFile(fileName);
      });
    ...
    cloudFilesList.innerHTML = `<p class="no-files">Error loading files: ${error.message}</p>`;
  }
}

async function downloadCloudFile(fileName) {
  try {
    const blob = await window.AzureStorage.downloadFromAzureBlob('excel-data', fileName);
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = fileName;
...
async function deleteCloudFile(fileName) {
  try {
    await window.AzureStorage.deleteFromAzureBlob('excel-data', fileName);
    await refreshCloudFiles();
  } catch (error) {
...
async function uploadExcelToCloud(fileData) {
  if (!window.AzureStorage || !window.AzureStorage.isAzureConfigured()) {
    return { success: false, error: 'Azure not configured' };
  }
  
  try {
    // Convert file data back to Excel blob
    const workbook = XLSX.utils.book_new();
    Object.keys(fileData.sheets).forEach(sheetName => {
      const ws = XLSX.utils.aoa_to_sheet(fileData.sheets[sheetName]);
    ...
    const result = await window.AzureStorage.uploadToAzureBlob(blob, 'excel-data', fileData.name);
    return result;
  } catch (error) {
...
async function uploadRecordingToCloud(recording, format) {
  if (!window.AzureStorage || !window.AzureStorage.isAzureConfigured()) {
    return { success: false, error: 'Azure not configured' };
  }
  ...
  try {
    ...
    const baseName = sanitizeFilename(recording.name);
    ...
    switch (format) {
      case 'json':
        ...
        break;
      case 'playwright':
        content = window.ExportGenerator?.generatePlaywright?.(recording) || '';
        filename = `${baseName}_playwright.js`;
        break;
      case 'selenium-python':
        content = window.ExportGenerator?.generateSeleniumPython?.(recording) || '';
        filename = `${baseName}_selenium.py`;
        break;
      case 'selenium-java':
        content = window.ExportGenerator?.generateSeleniumJava?.(recording) || '';
        filename = `${baseName}_selenium.java`;
        break;
      default:
        return { success: false, error: 'Unknown format' };
    }
    
    const result = await window.AzureStorage.uploadRecordingFile(content, filename);
    return result;
  } catch (error) {
    console.error('Upload recording to cloud failed:', error);
    return { success: false, error: error.message };
  }
}

// Upload all local files to Azure cloud
async function uploadAllFilesToCloud() {
  if (!window.AzureStorage || !window.AzureStorage.isAzureConfigured()) {
    alert('Please configure Azure Storage in Settings first');
    openSettingsModal();
    return;
  }
  ...
  const btn = document.getElementById('uploadAllToCloudBtn');
  ...
  for (const fileData of uploadedFiles) {
    const result = await uploadExcelToCloud(fileData);
    if (result.success) {
      successCount++;
    } else {
      failCount++;
      console.error('Failed to upload:', fileData.name, result.error);
    }
  }
  ...
  if (failCount === 0) {
    alert(`Successfully uploaded ${successCount} file(s) to Azure!`);
  } else {
    alert(`Uploaded ${successCount} file(s), failed ${failCount} file(s)`);
  }
}

// Update the Azure sync status indicator in the Data tab
function updateAzureSyncStatus() {
  const indicator = document.getElementById('azureSyncIndicator');
  const text = document.getElementById('azureSyncText');
  
  if (!indicator || !text) return;
  
  if (window.AzureStorage?.isAzureConfigured?.()) {
    indicator.classList.add('synced');
    text.textContent = `Cloud sync: Connected to ${window.AzureStorage.AZURE_CONFIG.accountName}`;
  } else {
    indicator.classList.remove('synced');
    text.textContent = 'Cloud sync: Not configured (Settings → Azure)';
  }
}
